...
#define DEFINE_BINARY_OP_FUNCTOR(lower, upper)                                    \
  template <typename T>                                                           \
  struct upper##Op {                                                              \
    __device__ T operator()(T a, T b) const { return BinaryOps<T>::lower(a, b); } \
  };

FOR_EACH_BINARY_OP(DEFINE_BINARY_OP_FUNCTOR)

template <typename scalar_t, typename Op>
void launchBinaryKernelImpl(scalar_t *out,
                            const scalar_t *a,
                            const scalar_t *b,
                            size_t n,
                            cudaStream_t stream) {
  ...
  void launch##upper##Kernel(void *out, const void *a, const void *b, size_t n,                    \
    ...
    switch (dtype) {                                                                               \
      case myops::MYOPS_DTYPE_FLOAT:                                                               \
        launchBinaryKernelImpl<float, upper##Op<float>>(static_cast<float *>(out),                 \
