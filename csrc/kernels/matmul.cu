...
__global__ void matmulKernel(scalar_t *__restrict__ c,
  ...
  if (row_idx < m && col_idx < n) {
    float sum = 0.0f;
    for (int i = 0; i < k; ++i) {
      sum += static_cast<float>(a[row_idx * k + i]) * static_cast<float>(b[i * n + col_idx]);
    }
...
void launchMatmulKernel(torch::Tensor C, torch::Tensor A, torch::Tensor B) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::ScalarType::Half, at::ScalarType::BFloat16, A.scalar_type(), "matmul", ([&] {
        matmulKernel<scalar_t><<<blocksPerGrid, threadsPerBlock, 0, stream>>>(
            C.data_ptr<scalar_t>(), A.data_ptr<scalar_t>(), B.data_ptr<scalar_t>(), m, n, k);
      }));
}